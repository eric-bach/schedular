...
export const GET_BOOKINGS = `query GetBookings($datetime: String!, $lastEvaluatedKey: LastEvaluatedKey) {
  getBookings(datetime: $datetime, lastEvaluatedKey: $lastEvaluatedKey) {
    items {
      pk
      sk
      type
...
export const GET_APPOINTMENTS_COUNTS = `query GetAppointmentCounts($type: String!, $from: String!, $to: String!) {
  getAppointmentCounts(type: $type, from: $from, to: $to)
  {
    date
    count
  }
